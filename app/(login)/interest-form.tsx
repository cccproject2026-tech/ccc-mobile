...
import { ChurchInfo } from "@/lib/user/types";
...
export default function InterestFormScreen() {
  ...
  const { submitInterest, isLoading } = useAuth();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Alert.alert("Error", "Failed to submit interest form. Please try again.");
    }
  };
            ...
            <React.Fragment key={church.id}>
              <View style={styles.section}>
                <Text style={styles.sectionTitle}>
                  Current Church Information
                  {formData.churches.length > 1 && ` #${index + 1}`}
                </Text>
                <TextInput
                  style={styles.input}
              ...
              {index < formData.churches.length - 1 && (
                <View style={styles.divider} />
              )}
            </React.Fragment>
          ))}
          ...
          <View style={styles.section}>
            ...
            >
              <Text
                style={[
                  styles.dropdownText,
                  formData.interests.length === 0 && styles.placeholderText,
                ]}
              >
