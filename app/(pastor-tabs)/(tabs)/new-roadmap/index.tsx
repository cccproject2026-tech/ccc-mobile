...
//             // Calculate phase status (same logic as usePhaseToCard)
...
//                 return i.dueDate && i.dueDate <= today && actualStatus !== 'COMPLETED';
//             });
...
//         })
//         .filter(({ phase, phaseStatus }) => {
//             if (activeTab === 'ALL') return true;

//             // Map tab names to phase status
//             if (activeTab === 'COMPLETED') return phaseStatus === 'completed';
//             if (activeTab === 'IN_PROGRESS') return phaseStatus === 'in-progress';
//             if (activeTab === 'NOT_STARTED') return phaseStatus === 'initial';
//             if (activeTab === 'DUE') return phaseStatus === 'due';
...
// app/(tabs)/new-roadmap/index.tsx - CLEAN & OPTIMIZED
import ContextMenu, { MenuItem } from '@/components/director/ContextMenu';
import ExpectedOutcomeModal from '@/components/director/ExpectedOutcomeModal';
import RoadmapCard from '@/components/director/ProgressRoadmapCard';
import SearchBar from '@/components/director/SearchBar';
...
export default function PhaseList() {
    ...
    const handlePhasePress = useCallback((phase: typeof filteredPhases[0]['phase']) => {
        if (phase.isSingleRoadmap && phase.tasks.length === 1) {
