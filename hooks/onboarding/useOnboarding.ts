...
export const useCheckApprovalStatus = (enabled: boolean = false) => {
    ...
    useEffect(() => {
        ...
        if (queryResult.isError) {
            // @ts-ignore
            console.error('❌ Check approval status failed:', queryResult.error?.message);
        }
    }, [queryResult.isSuccess, queryResult.isError, queryResult.data, interestStatus, setInterestStatus]);
